    ...
    if (pgdir[PDX(va)] == 0 || !((pgdir[PDX(va)] & PTE_P))) {
        if (create) {
            ...
            if (pi == NULL)
                return NULL;
            pi->pp_ref++;
            pgdir[PDX(va)] = (page2pa(pi) | PTE_P | PTE_W);
        } else
            return NULL;
    }
    pte_t *entry = (pte_t *) (KADDR(PTE_ADDR(pgdir[PDX(va)])));
    assert(entry);
    return (pte_t *) (&entry[PTX(va)]);
}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm) {
    ...
    for (uint32_t offset = 0; offset < size; offset += PGSIZE) {
        if (!pgdir[PDX(va + offset)]) {
            struct PageInfo *pi = page_alloc(ALLOC_ZERO);
            if (pi == NULL) {
                panic("what!@#");
            }
            pgdir[PDX(va + offset)] = page2pa(pi) | PTE_P | PTE_U | perm;
            pi->pp_ref++;
        }
        pte_t *tentry = pgdir_walk(pgdir, (void *) (va + offset), true);
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm) {
    if (!pgdir[PDX(va)]) {
        ...
        pgdir[PDX(va)] = page2pa(pi) | PTE_P | PTE_U | perm;
        pi->pp_ref++;
    }
...
//    cprintf("start:%x,len:%x\n",va,va+len);
    user_mem_check_addr=(uintptr_t)va;
    while (start < end) {
        if (start >= (void *) ULIM)
            return -E_FAULT;
        pte_t *entry = pgdir_walk(env->env_pgdir, start, 0);
        if (!entry)
            return -E_FAULT;
        uint32_t p = *entry & ((1 << 12) - 1);
//        cprintf("p:%x\n",p);
        if ((p & perm)!=perm)
            return -E_FAULT;
        start += PGSIZE;
